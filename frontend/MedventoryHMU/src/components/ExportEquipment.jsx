...
export default function ExportEquipment({ equipmentData, onExport }) {
  ...
  function exportFilteredData() {
    ...
    const csv = [headers.join(","), ...data.map(eq => [eq.code, `"${eq.name}"`, `"${eq.department}"`, `"${eq.status}"`, eq.date, eq.value].join(","))].join("\n");
  ...
  function quickExport(type) {
    ...
    setTimeout(() => { updatePreview(); previewExport(); }, 0);
  }
    ...
    <div className="ee-root card">
      ...
      <div className="ee-grid">
        <div className="ee-filters">
          ...
          <div className="ee-field">
            ...
            <div className="ee-radios">
              <label><input type="radio" name="fmt" defaultChecked /> <span>📊 Excel (.xlsx)</span></label>
              <label><input type="radio" name="fmt" /> <span>📄 CSV (.csv)</span></label>
              <label><input type="radio" name="fmt" /> <span>📋 PDF (.pdf)</span></label>
            </div>
          </div>

          <div className="ee-actions">
            <button className="btn outline" onClick={previewExport}>👁️ Xem trước</button>
            <button className="btn primary" onClick={exportFilteredData}>📤 Xuất dữ liệu</button>
          </div>
        </div>
        ...
        <div className="ee-stats">
          ...
          <div className="ee-statsbox">
            <div className="row"><span>Tổng số vật tư:</span><span>{stats.total}</span></div>
            <div className="row"><span>Hoạt động tốt:</span><span className="green">{stats.working}</span></div>
            <div className="row"><span>Cần bảo trì:</span><span className="yellow">{stats.maintenance}</span></div>
            <div className="row"><span>Hỏng hóc:</span><span className="red">{stats.broken}</span></div>
          </div>
      ...
      <div id="export-preview" className={`ee-preview ${previewData.length ? "visible" : "hidden"}`}>
        ...
        <div className="ee-preview-table">
          <table>
            <thead>
              <tr><th>Mã TB</th><th>Tên vật tư</th><th>Khoa phòng</th><th>Trạng thái</th><th>Ngày mua</th></tr>
            </thead>
            <tbody>
              {previewData.length ? previewData.map(eq => (
                <tr key={eq.id}>
                  <td>{eq.code}</td>
                  <td>{eq.name}</td>
                  <td>{eq.department}</td>
                  <td><span className={`pill ${eq.status === "Hoạt động tốt" ? "g" : eq.status === "Cần bảo trì" ? "y" : "r"}`}>{eq.status}</span></td>
                  <td>{eq.date}</td>
                </tr>
